...
class Sanitize
{
     ...
     * @return string
     */
    public static function editor(string $input, bool $force_filter = false, bool $auto_p = false, array $allowed_tags = null): string
    {
        if (current_user_can('unfiltered_html') && !$force_filter) {
            $output = trim($input);
        } else {
            global $allowedtags;
            $output = wp_kses(trim($input), apply_filters('plugin_sanitize_editor_tags', $allowed_tags ?? $allowedtags));
        }
